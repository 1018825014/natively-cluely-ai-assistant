...
class OpenAIStreamingSTT extends events_1.EventEmitter {
    ...
    apiKey;
    languageKey = 'en';
    technicalGlossaryConfig = (0, TechnicalGlossary_1.normalizeTechnicalGlossaryConfig)();
    ...
    isSessionReady = false; // set after transcription_session.created
    ...
    _connectWs() {
        ...
        this.ws = new ws_1.default(REALTIME_WS_URL, {
            headers: {
    ...
    _closeWs(graceful) {
        if (!this.ws)
            return;
        try {
            if (graceful && this.ws.readyState === ws_1.default.OPEN) {
    ...
    async _restUpload(wavBuffer) {
        ...
        const response = await axios_1.default.post(REST_ENDPOINT, form, {
            headers: {
                Authorization: `Bearer ${this.apiKey}`,
                ...form.getHeaders(),
            },
    ...
    getTranscriptionPrompt() {
        return (0, TechnicalGlossary_1.buildOpenAITranscriptionPrompt)(this.technicalGlossaryConfig);
    }
}
