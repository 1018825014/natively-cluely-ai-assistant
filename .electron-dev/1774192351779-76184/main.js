...
class AppState {
    ...
    notifySpeakerSpeechEnded(speaker) {
        const timestamp = Date.now();
        if (speaker === 'interviewer') {
            this.intelligenceManager.commitLiveTranscriptSegment();
            if (this.intelligenceManager.hasEditedLiveTranscript()) {
                void this.resyncLiveMeetingRag().catch((error) => {
    ...
    startCompareShadowProviders() {
        ...
        const currentProvider = this.getCurrentSttProviderId();
        const candidates = ['openai', 'alibaba'];
    ...
    setTechnicalGlossaryConfig(config) {
        CredentialsManager_1.CredentialsManager.getInstance().setTechnicalGlossaryConfig(config);
