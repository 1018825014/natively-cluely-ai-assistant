...
class SessionTracker {
    ...
    static TRANSCRIPT_SEPARATOR_REGEX = /[\s\u3002\uFF01\uFF1F\uFF1B\uFF1A\uFF0C,.!?;:]/;
    static TRANSCRIPT_PUNCTUATION_GLOBAL_REGEX = /[\u3002\uFF01\uFF1F\uFF1B\uFF1A\uFF0C,.!?;:]/g;
    contextItems = [];
    ...
    maybeCommitLiveTranscriptSegment(id, speaker = 'interviewer') {
        const target = id ? this.liveTranscriptSegments.find(segment => segment.id === id) : this.getActiveLiveSegment(speaker);
        if (!target || target.status === 'final')
            return null;
    ...
    mergeLiveTranscriptSegmentWithPrevious(id) {
        ...
        this.setLastInterimSegment(previous.speaker, nextStatus === 'active' ? this.toTranscriptSegment(previous, false) : null);
        if (nextStatus === 'final') {
    ...
    getTranscriptForRag(includeActiveInterviewer = true) {
        ...
        if (includeActiveInterviewer) {
            ...
            for (const interim of interimSegments) {
                const isDuplicate = segments.some((item) => item.speaker === interim.speaker &&
    ...
    getContext(lastSeconds = 120) {
        ...
        const interimSegments = this.getInterimTranscriptSegments();
        for (const interim of interimSegments) {
            if (!interim.text.trim() || interim.timestamp < cutoff)
                continue;
            const role = this.mapSpeakerToRole(interim.speaker);
            const duplicate = items.some(item => item.role === role &&
                Math.abs(item.timestamp - interim.timestamp) < 500 &&
                this.normalizeTranscriptForComparison(item.text) === this.normalizeTranscriptForComparison(interim.text));
            if (!duplicate) {
    ...
    buildNormalizedTranscriptMap(text) {
    ...
    extractTailAfterTranscriptOverlap(baseText, incomingText) {
        ...
        if (!baseMap.normalized || !incomingMap.normalized)
            return null;
    ...
    shouldFinalizeLiveTranscriptSegment(segment) {
        return (this.normalizeTranscriptForComparison(segment.text).length >= SessionTracker.SOFT_INTERVIEWER_SEGMENT_CHARS &&
            SessionTracker.SENTENCE_BOUNDARY_REGEX.test(segment.text.trim()));
    }
    splitLiveTranscriptIntoChunks(text) {
    ...
    syncCommittedInterviewerSegment(segment) {
        ...
        for (let index = this.liveTranscriptSegments.length - 1; index >= 0; index -= 1) {
            ...
            if (item.id === segment.id || item.speaker !== segment.speaker || item.status !== 'final')
                continue;
            if (Math.abs(item.updatedAt - segment.updatedAt) > SessionTracker.RECENT_DUPLICATE_INTERVIEWER_WINDOW_MS)
                continue;
