...
class SessionTracker {
    ...
    applyLiveSegmentState(current, options) {
        ...
        if (chunks.length === 1) {
            ...
            if (options.isIncomingFinal &&
                this.normalizeTranscriptForComparison(current.text).length >= SessionTracker.SOFT_INTERVIEWER_SEGMENT_CHARS &&
                SessionTracker.SENTENCE_BOUNDARY_REGEX.test(current.text)) {
                this.finalizeLiveTranscriptSegment(current.id, options.timestamp);
                return;
            }
