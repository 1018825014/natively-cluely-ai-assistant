...
class ScreenshotHelper {
     ...
     */
    async captureWithDesktopCapturer(outputPath, area) {
        let targetDisplay;
        if (area) {
        ...
        try {
            ...
            if (error.name === 'NotAllowedError') {
                throw new Error('Screen capture permission denied. Please grant screen recording permission in System Settings > Privacy & Security > Screen Recording.');
            }
        ...
        if (sources.length === 0) {
            ...
            throw new Error('No screen sources available. Check screen recording permissions in System Settings > Privacy & Security > Screen Recording.');
        }
    ...
    async takeSelectiveScreenshot(hideMainWindow, showMainWindow, captureArea) {
        try {
            ...
            if (process.platform === 'win32' && captureArea) {
                ...
                if (isMulti) {
                    ...
                    await node_fs_1.default.promises.writeFile(screenshotPath, stitchedBuffer);
