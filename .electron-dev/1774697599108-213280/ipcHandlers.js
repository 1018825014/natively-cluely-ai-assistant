...
function initializeIpcHandlers(appState) {
    ...
    safeHandle("live-transcript:merge-with-previous", async (_event, payload) => {
        const intelligenceManager = appState.getIntelligenceManager();
    ...
    safeHandle("open-prompt-lab-window", async (_event, payload) => {
        const action = payload?.action || "what_to_answer";
        appState.openPromptLabWindow(action, payload?.context);
        return { success: true };
    });
    safeHandle("prompt-lab:get-action-preview", async (_event, action, context) => {
        return appState
            .getPromptLabService()
    ...
    safeHandle("set-phone-interviewer-mode", async (_, enabled) => {
    ...
    // ==========================================
    safeHandle("profile:preview-resume-import", async (_, payload) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
    ...
    safeHandle("projectLibrary:updateProject", async (_, project) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
                return { success: false, error: 'Knowledge engine not initialized' };
    ...
    safeHandle("projectLibrary:getProjectDetail", async (_, projectId) => {
        const orchestrator = appState.getKnowledgeOrchestrator();
        if (!orchestrator)
            return null;
        return orchestrator.getProjectDetail(projectId);
    });
    safeHandle("projectLibrary:listAssets", async (_, projectId) => {
        const orchestrator = appState.getKnowledgeOrchestrator();
        if (!orchestrator)
            return [];
        return orchestrator.listProjectAssets(projectId);
    });
    safeHandle("projectLibrary:updateAssetText", async (_, payload) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
                return { success: false, error: 'Knowledge engine not initialized' };
    ...
    safeHandle("projectLibrary:deleteAsset", async (_, assetId) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
                return { success: false, error: 'Knowledge engine not initialized' };
    ...
    safeHandle("projectLibrary:listRepos", async (_, projectId) => {
        const orchestrator = appState.getKnowledgeOrchestrator();
        if (!orchestrator)
            return [];
        return orchestrator.listRepos(projectId);
    });
    safeHandle("projectLibrary:replaceRepo", async (_, payload) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
                return { success: false, error: 'Knowledge engine not initialized' };
    ...
    safeHandle("projectLibrary:reindexRepo", async (_, payload) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
                return { success: false, error: 'Knowledge engine not initialized' };
    ...
    safeHandle("projectLibrary:deleteRepo", async (_, payload) => {
        try {
            const orchestrator = appState.getKnowledgeOrchestrator();
            if (!orchestrator) {
                return { success: false, error: 'Knowledge engine not initialized' };
