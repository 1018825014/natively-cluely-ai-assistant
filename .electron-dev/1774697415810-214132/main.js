...
function formatHotwordSyncTarget(targetModel) {
    switch (targetModel) {
        case AlibabaHotwordSync_1.PARAFORMER_REALTIME_TARGET_MODEL:
            return "阿里云 Paraformer";
        case AlibabaHotwordSync_1.FUN_ASR_REALTIME_TARGET_MODEL:
            return "Fun-ASR";
        default:
            return targetModel;
    }
}
function formatTechnicalGlossarySyncWarning(warning) {
...
function formatTechnicalGlossarySyncWarnings(warnings) {
...
class AppState {
    ...
    phoneInterviewerMode = false;
    isManualAnswerCaptureActive = false;
    ...
    gracefulQuitPromise = null;
    installUpdatePromise = null;
    overlayClosePromise = null;
    hasPreparedForQuit = false;
    isQuitRequested = false;
    allowNativeQuit = false;
    ...
    async stopRealtimeSessionResources() {
    ...
    async handleOverlayCloseRequest() {
    ...
    getPhoneInterviewerMode() {
        return this.phoneInterviewerMode;
    }
    setPhoneInterviewerMode(enabled) {
        ...
        SettingsManager_1.SettingsManager.getInstance().set('phoneInterviewerMode', enabled);
    ...
    shouldUsePhoneInterviewerMode() {
        return this.phoneInterviewerMode;
    }
    shouldRouteSystemAudioToInterviewer() {
        return !this.shouldUsePhoneInterviewerMode();
    }
    getCurrentMicLogicalSpeaker() {
        return this.shouldUsePhoneInterviewerMode() && !this.isManualAnswerCaptureActive
            ? 'interviewer'
            : 'user';
    }
    getInterviewerPipelineSampleRate() {
    ...
    getUserPipelineSampleRate() {
        return this.microphoneCapture?.getSampleRate() || 48000;
    }
    refreshPrimarySttSampleRates() {
