...
class SessionTracker {
    ...
    handleLiveSpeakerTranscript(segment) {
        ...
        if (this.shouldStartNewInterviewerSegment(current, nextText, timestamp)) {
            const nextSegmentText = this.getNextSplitSegmentText(speaker, current, nextText);
            this.finalizeInterviewerSegment(current.id, current.updatedAt);
            if (!nextSegmentText) {
                return null;
            }
            if (this.hasRecentSpeakerDuplicate(speaker, nextSegmentText, timestamp)) {
                return null;
            }
            const liveSegment = {
                id: this.createSegmentId(),
    ...
    getNextSplitSegmentText(speaker, current, nextProviderText) {
        const tailText = this.stripCommittedPrefixFromIncoming(nextProviderText, [
            current.lastProviderText,
            current.text,
