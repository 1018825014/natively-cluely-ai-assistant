...
exports.TraceWindowHelper = void 0;
...
const DEFAULT_TRACE_WIDTH = 1200;
const DEFAULT_TRACE_HEIGHT = 820;
const MIN_TRACE_WIDTH = 900;
const MIN_TRACE_HEIGHT = 640;
class TraceWindowHelper {
    window = null;
    ...
    createWindow(anchorWindow) {
        ...
        this.window.loadURL(`${startUrl}?window=trace`).catch(error => {
            console.error("[TraceWindowHelper] Failed to load Trace window:", error);
        });
    ...
    getInitialBounds(anchorWindow) {
        ...
        const width = Math.min(DEFAULT_TRACE_WIDTH, workArea.width);
        const height = Math.min(DEFAULT_TRACE_HEIGHT, workArea.height);
    ...
    ensureVisibleOnScreen(anchorWindow) {
        ...
        const { workArea } = display;
        const width = Math.min(Math.max(currentBounds.width, MIN_TRACE_WIDTH), workArea.width);
        const height = Math.min(Math.max(currentBounds.height, MIN_TRACE_HEIGHT), workArea.height);
...
exports.TraceWindowHelper = TraceWindowHelper;
//# sourceMappingURL=TraceWindowHelper.js.map