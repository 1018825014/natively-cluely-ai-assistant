...
class LLMHelper {
  ...
  "solution": {
    ...
    async generateSuggestion(context, lastQuestion) {
        return this.generateWithInternalTextFallback(`You are an expert interview coach. Based on the conversation transcript, provide a concise, natural response the user could say.
...
- Never hedge.
- Never say "it depends".`, `CONVERSATION SO FAR:\n${context}\n\nLATEST QUESTION FROM INTERVIEWER:\n${lastQuestion}\n\nANSWER DIRECTLY:`, {
...
- Do not add explanations, prefaces, commentary, or markdown fences unless the task explicitly requests them.
- If the task asks for JSON, return only valid JSON.
- If the task asks for a title, return only the title text.
- If the task asks for one sentence, return exactly one sentence.`;
