...
class SessionTracker {
    ...
    handleLiveSpeakerTranscript(segment) {
        ...
        const latestCommitted = this.getLatestCommittedLiveSegment(speaker);
        const canonicalIncomingText = this.stripCommittedPrefixFromIncoming(nextText, latestCommitted?.text);
        if (!current && latestCommitted) {
            if (!canonicalIncomingText && this.shouldIgnoreRecentInterviewerDuplicate(latestCommitted, nextText, timestamp)) {
                return null;
            }
            if (canonicalIncomingText && this.shouldIgnoreRecentInterviewerDuplicate(latestCommitted, canonicalIncomingText, timestamp)) {
                return null;
            }
        }
        ...
        if (!current) {
            const liveSegment = {
                ...
                edited: false,
                lastProviderText: effectiveIncomingText,
        ...
        if (this.shouldStartNewInterviewerSegment(current, effectiveIncomingText, timestamp)) {
            this.finalizeInterviewerSegment(current.id, current.updatedAt);
            return this.handleLiveSpeakerTranscript({
                ...segment,
                text: effectiveIncomingText,
            });
        }
        current.text = current.edited
            ? this.appendTailWithoutOverwritingEdits(current, effectiveIncomingText)
            : this.mergeProviderTranscript(current.lastProviderText, effectiveIncomingText, current.text);
        current.updatedAt = Math.max(timestamp, current.updatedAt);
        current.lastProviderText = effectiveIncomingText;
    ...
    shouldIgnoreRecentInterviewerDuplicate(recentSegment, nextText, timestamp) {
        if (!recentSegment.text.trim() || !nextText.trim())
            return false;
        if (Math.abs(timestamp - recentSegment.updatedAt) > SessionTracker.RECENT_DUPLICATE_INTERVIEWER_WINDOW_MS)
            return false;
        return this.normalizeTranscriptForComparison(recentSegment.text) === this.normalizeTranscriptForComparison(nextText);
    }
    getLatestCommittedLiveSegment(speaker) {
        for (let index = this.liveTranscriptSegments.length - 1; index >= 0; index -= 1) {
            if (this.liveTranscriptSegments[index].speaker === speaker && this.liveTranscriptSegments[index].status === 'final') {
                return this.liveTranscriptSegments[index];
            }
        }
        return undefined;
    }
    stripCommittedPrefixFromIncoming(nextText, previousCommittedText) {
        const incoming = nextText.trim();
        const committed = previousCommittedText?.trim();
